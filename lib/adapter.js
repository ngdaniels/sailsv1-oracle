...
module.exports = (function() {
    ...
    var adapter = {
        ...
        create: function(connectionName, table, cb) {
            ...
            //remove null parameters and parameter with object type
            Object.keys(table.newRecord).forEach(key => {
                if (key !== 'createdAt' || key !== 'updatedAt') {
                    if (table.newRecord[key] === null) {
                        delete table.newRecord[key];
                    }
                    else if (typeof table.newRecord[key] === 'object') {
                        console.log(table.newRecord[key]);
                        delete table.newRecord[key];
                    }
                }
            });
            ...
            query.bindings.forEach((binding, index) => {
                let date_format = /^(-?(?:[1-9][0-9]*)?[0-9]{4})-(1[0-2]|0[1-9])-(3[01]|0[1-9]|[12][0-9])T(2[0-3]|[01][0-9]):([0-5][0-9]):([0-5][0-9])(.[0-9]+)?(Z)?$/g
                let check = String(binding).match(date_format);
            ...
            connectionObject.pool.getConnection(
                function(err, connection) {
                    ...
                    connection.execute(query.sql, query.bindings, { /*outFormat: oracledb.OBJECT, */
                        ...
                        var selectQuery = 'select * from "' + table.using + '" order by "createdAt" desc';
        ...
        update: function(connectionName, table, cb) {
            ...
            //remove null parameters and parameter with object type
            Object.keys(table.valuesToSet).forEach(key => {
                if (key !== 'createdAt' || key !== 'updatedAt') {
                    if (table.valuesToSet[key] === null) {
                        delete table.valuesToSet[key];
                    }
                    else if (typeof table.valuesToSet[key] === 'object') {
                        console.log(table.valuesToSet[key]);
                        delete table.valuesToSet[key];
                    }
                }
            });
